...
- 识别并清理已经结束的子代理：保护当前主对话和运行中的代理，先做轻量 runtime preflight，通过后才生成清单和临时数据库快照，再通过官方 `thread/delete` 先单个验证、后分批删除。
- 将删除运行时与当前桌面 app-server 做精确哈希和 OpenAI 签名配对，并用机器可读矩阵核对 migration 锚点、对象指纹、canary 原始结果和备份有效性，避免把旧修复误用到新版本。
...
先做轻量只读 preflight。它只读取当前桌面 app-server、候选镜像的文件身份和少量 migration 锚点，不扫描全部 rollout，也不创建数据库备份：
...
只有结果明确包含 `allow_expensive_inventory: true` 才继续。镜像路径本身不构成信任：通常使用的 `D:\CodexHome\plugins\.plugin-appserver\codex.exe` 必须与当前运行中的桌面 app-server 大小和 SHA-256 完全一致，Authenticode 必须为有效且签发给 OpenAI。版本、哈希、签名、进程来源或 migration 锚点任一不完整，都在昂贵清单与备份之前停止。

通过 gate 后再生成只读清单，并把当前主任务和每个运行中子代理的 ID 都加入 `--protect`：

```powershell
New-Item -ItemType Directory -Force 'C:\Codex-cleanup-manifests' | Out-Null
...
该脚本只纳入有明确 spawn 关系、代理元数据或 rollout 标记的记录；证据不足的记录会被排除。完整顺序是：

1. preflight 允许后，重新查询当前任务和运行中的代理，更新保护 ID；
2. 生成清单，核对候选根任务、子树数量、精确字节数和最近修改时间；
3. 通过 SQLite 在线备份 API，将 `state_5.sqlite`、`goals_1.sqlite` 和 `memories_1.sqlite` 临时备份到 `CodexHome` 外；不复制全部 rollout；
4. 使用同一份新鲜 runtime evidence 和备份摘要运行完整只读诊断；
5. 只用 runtime evidence 指定的官方运行时，对一个候选根任务执行 canary；诊断不能代替 canary；
6. native canary 一旦失败，保存未经改写的报错和部分删除状态后立即停止，不能转用旧 shim，也不能自动重试；legacy `0.142.2` 只处理在本流程外已经存在并精确命中的旧事故；
7. canary 成功后，通过同一运行时的 `app-server --stdio` `thread/delete` 逐根删除；每次官方调用前都重新运行 preflight、要求 `condition_key` 不变，并复核实时子树、保护 ID 和清单后的文件修改；
8. 验证受保护任务仍可打开、候选文件和索引记录已经消失、各数据库 `quick_check` 为 `ok`，再继续下一批；
9. 锁定、RPC/超时、保护范围变化或任何新的部分删除都立即停止；全部验证通过后再移除本次临时兼容对象。

不要逐个删除同一子树的所有后代，也不要把闲置主对话或归档对话误判为子代理。更推荐让技能完成实时核对、canary 和结果验证，而不是手工批量执行删除命令。

#### 删除兼容性：版本正确也不能跳过检查

版本号本身不能证明“有问题”或“已修复”。native 路径需要同时核对：

- 当前运行桌面 app-server 的包路径、进程身份和精确文件哈希；
- 可访问镜像与运行时的大小和 SHA-256 完全一致，并具有有效 OpenAI Authenticode 签名；
- 运行时版本至少为 `0.145.0`，migration 14、15、42 锚点精确且没有失败 migration；
- 4 个临时兼容对象全部不存在；
- 同一运行时执行的官方 canary 成功；
- 三个外部在线备份的时间、路径、大小、SHA-256 和实时 `quick_check`。

preflight 通过后，使用它的原始输出运行只读诊断：
...
只有在恢复一个已经存在的 legacy `0.142.2` 精确部分删除事故时，才把原始证据和新鲜备份摘要传入下列命令；native canary 失败后禁止使用：
...
| 结果 | 含义和处理 |
| --- | --- |
| `canary_required` | runtime 和数据库证据允许；只用已验证运行时做一个官方 canary，不能预先建表 |
...
正常路径是与当前桌面 app-server 精确配对的 native 运行时。当前唯一已验证的临时方案只覆盖 legacy `codex 0.142.2`、migration 14/15/42 锚点、独立证书中精确的 43/44 tail，以及 `no such table: agent_jobs` canary 指纹。证书只接受两个固定 migration、固定 SHA-384、固定 40 位 OpenAI commit URL 和固定审查影响；不接受范围、通配符或配置文件自我声明“已审”。它不是“只要缺 `agent_jobs` 就建表”的通用修复，也绝不能用于 native canary 失败。
...
兼容矩阵和 legacy tail certificate 都带有 `review_after`。当前复核截止日是 `2026-11-01`；到期、运行时无法与桌面进程精确配对、签名无效、migration 锚点或 legacy tail 未收录、删除报错不同或官方实现变化时，现场删除会停止并提示更新技能。
...
更新完成后必须重新运行只读 preflight 和完整诊断；不能自动续跑上次失败的删除。

定时任务遇到相同的 `unsupported_update_required` 不应每三小时重复扫描 rollout 和生成备份。它应在 preflight 阶段暂停，用稳定的 `condition_key` 去重通知，并保留第一次证据。脚本不会自行切换自动任务状态；安装已审更新并手工完成 preflight/完整诊断验证后，由操作员通过 Codex 自动化控制重新启用，恢复后的运行再做新的 canary。
...
5. 子代理清理单独确认，先做轻量 preflight；只有 gate 允许后才做清单、备份、完整诊断和 canary，并在每批删除前重新核对活动状态；
...
├─ references/
│  ├─ subagent-delete-compatibility.json  # native 策略、legacy 指纹和 DDL 哈希
│  ├─ subagent-delete-tail-certificates.json # legacy 精确已审 migration tail
